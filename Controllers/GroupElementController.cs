...
[Authorize]
public class GroupElementController : ControllerBase
{
    private readonly DataContext _context;

    public GroupElementController(DataContext context)
    {
        _context = context;
    }
    ...
    [HttpPut]
    [Authorize(Roles = UserRole.Admin)]
    public async Task<ActionResult<ElementGroup>> Update([FromBody] UpdateDto dto)
    {
        var type = await _context.ElementGroups.FindAsync(dto.Id);
    ...
    {
        var type = await _context.ElementGroups.FindAsync(id);
    ...
    {
        var types = await _context.ElementGroups.Include(type => type.User).AsNoTracking().ToListAsync();
        return Ok(types);
    }
    ...
    {
        var type = await _context.ElementGroups.Include(elementType => elementType.User)
